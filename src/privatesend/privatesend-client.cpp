...
bool CPrivateSendClientSession::SignFinalTransaction(const CTransaction& finalTransactionNew, CNode* pnode, CConnman& connman)
{
    ...
    for (const auto& entry : vecEntries) {
        ...
        for (const auto& txdsin : entry.vecTxDSIn) {
            ...
            // TODO we're using amount=0 here but we should use the correct amount. This works because BiblePay ignores the amount while signing/verifying (only used in Bitcoin/Segwit)
...
bool CPrivateSendClientSession::MakeCollateralAmounts(const CompactTallyItem& tallyItem, bool fTryDenominated)
{
    ...
    if (txBuilder.CouldAddOutputs({CPrivateSend::GetMaxCollateralAmount(), CPrivateSend::GetCollateralAmount()})) {
        ...
        // If remainder is denominated add one pence to the fee
        ...
        // Create two equal outputs from the available value. This adds one pence to the fee if txBuilder.GetAmountLeft() is odd.
