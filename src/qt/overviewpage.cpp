...
public:
    explicit TxViewDelegate(QObject* parent = nullptr) :
        QAbstractItemDelegate(), unit(BitcoinUnits::BBP)
    {
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
	...
	// BIBLEPAY - Show USD balance
	double dUSD = GetUSDValueBBP(balance + unconfirmedBalance + immatureBalance);
    ui->lblBalanceUSD->setText(GUIUtil::TOQS("$" + RoundToString(dUSD, 2)));
...
void OverviewPage::privateSendStatus(bool fForce)
{
	...
	// BBP: Update Prayers on Overview Page
	PRAYER_MODULUS++;
