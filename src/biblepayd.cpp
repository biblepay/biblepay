...
bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/biblepay.confare parsed in qt/biblepay.cpp's main()
    SetupServerArgs();
#if HAVE_DECL_DAEMON
    ...
    if (gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") ||  gArgs.IsArgSet("-help") || gArgs.IsArgSet("-version"))
    {
        ...
        {
            strUsage += "\nUsage:\n"
                  "  biblepayd [options]                     " + strprintf("Start %s Daemon", PACKAGE_NAME) + "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                fprintf(stderr, "Error: Command line contains unexpected token '%s', see biblepayd -h for a list of options.\n", argv[i]);
                return false;
            }
        }
