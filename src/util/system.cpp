...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //biblepay-tx using stdin
...
{
    // ~/.biblepaycore
    // Windows: C:\Users\Username\AppData\Roaming\BiblePayCore
    // macOS: ~/Library/Application Support/BiblePayCore
    // Unix-like: ~/.biblepaycore
#ifdef WIN32
    // Windows
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good())
    {
        ...
        // Create an empty biblepay.conf if it does not exist
        std::string sOpenPath = GetConfigFile(confPath).string().c_str();
        LogPrintf("CONFIGFILE::Defaulting to create %s",sOpenPath);
...
//    std::string strCopyrightHolders = strPrefix + strprintf(" %u-%u ", nStartYear, nEndYear) + strprintf(_(COPYRIGHT_HOLDERS).translated, _(COPYRIGHT_HOLDERS_SUBSTITUTION));
    ...
    // Check for untranslated substitution to make sure BiblePay Core copyright is not removed by accident
    if (copyright_devs.find("BiblePay Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2014, nEndYear) + "The BiblePay Core developers";
    }
