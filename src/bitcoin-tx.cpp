...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " biblepay-tx utility version " + FormatFullVersion() + "\n";
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  biblepay-tx [options] <hex-tx> [commands]  Update hex-encoded biblepay transaction\n"
                "or:     biblepay-tx [options] -create [commands]   Create hex-encoded biblepay transaction\n"
                "\n";
