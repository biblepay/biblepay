...
- [Development guidelines](#development-guidelines)
    - [General BiblePay Core](#general-biblepay-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
BiblePay Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
BiblePay Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/biblepay/biblepay/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_biblepay
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_biblepay --log_level=test_suite
$ valgrind -v --leak-check=full src/biblepayd -printtoconsole
...
# A coverage report will now be accessible at `./test_biblepay.coverage/index.html`.
```
...
To profile a running biblepayd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep biblepayd` -- sleep 60
```
...
BiblePay Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`biblepayd`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `biblepay-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `biblepay-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of BiblePay Core, the wallet was always
...
- Use *invalid* BiblePay addresses (e.g. in the constant array `EXAMPLE_ADDRESS`) for
