...
static bool ContextualCheckTransaction(const CTransaction& tx, TxValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (fDIP0001Active_context && ::GetSerializeSize(tx, PROTOCOL_VERSION) > MAX_STANDARD_TX_SIZE) {
        return state.Invalid(TxValidationResult::TX_CONSENSUS, "bad-txns-oversize-2");
    }
...
static std::pair<CAmount, CAmount> GetBlockSubsidyHelper(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fV20Active)
{
    ...
    if (nSubsidyBase > 20000)
        nSubsidyBase = 20000;
    else if (nSubsidyBase < 10)
        nSubsidyBase = 10;

    // This rule allows us to take out all of the business logic we had between 2017-2021

    if (nPrevHeight < consensusParams.BARLEY_HARVEST_HEIGHT) {
        CAmount nMaxSubsidy = MAX_BLOCK_SUBSIDY * COIN;
        return {nMaxSubsidy,nMaxSubsidy};
    }
    ...
    return {nNetSubsidy, nSuperblockPart};
}
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue, bool fActive)
{
    // https://wiki.bible[pay].org/Economics
...
static int64_t nTimeBiblepaySpecific = 0;
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, m_mnhfManager, *m_quorum_block_processor, *m_clhandler, m_params.GetConsensus(), view, fJustCheck, fScriptChecks, state, mnlist_updates_opt)) {
        return error("ConnectBlock(BIBLEPAY): ProcessSpecialTxsInBlock for block %s failed with %s",
        ...
        for (const auto& tx : block.vtx) {
            ...
            while (llmq::CInstantSendLockPtr conflictLock = m_isman->GetConflictingLock(*tx)) {
                if (m_clhandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                    LogPrint(BCLog::ALL, "ConnectBlock(BIBLEPAY): chain-locked transaction %s overrides islock %s\n",
                    ...
                    LogPrintf("ERROR: ConnectBlock(BIBLEPAY): transaction %s conflicts with transaction lock %s\n", tx->GetHash().ToString(), conflictLock->txid.ToString());
    ...
    if (!CheckCreditPoolDiffForBlock(block, pindex, m_params.GetConsensus(), blockSubsidy, state)) {
        return error("ConnectBlock(BIBLEPAY): CheckCreditPoolDiffForBlock for block %s failed with %s",
                     pindex->GetBlockHash().ToString(), state.ToString());
    }
    ...
    if (!MasternodePayments::IsBlockValueValid(*sporkManager, *governance, *::masternodeSync, block, pindex->nHeight, blockSubsidy + feeReward, strError)) {
        ...
        LogPrintf("ERROR: ConnectBlock(BIBLEPAY): %s\n", strError);
    ...
    if (!MasternodePayments::IsBlockPayeeValid(*sporkManager, *governance, *::masternodeSync, *block.vtx[0], pindex->pprev, blockSubsidy, feeReward)) {
        // NOTE: Do not punish, the node might be missing governance data
        LogPrintf("ERROR: ConnectBlock(BIBLEPAY): couldn't find masternode or superblock payments\n");
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeBiblepaySpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - BiblePay specific: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime5 - nTime4), nTimeBiblepaySpecific * MICRO, nTimeBiblepaySpecific * MILLI / nBlocksTotal);
...
static bool CheckBlockHeader(const CBlockHeader& block, const uint256& hash, BlockValidationState& state,
    const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    // Check proof of work matches claimed amount
    if (fCheckPOW && !CheckProofOfWork(hash, block.nBits, consensusParams, block.nVersion, block.nTime))
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (block.nBits != GetNextWorkRequired(pindexPrev, &block, consensusParams)) {
        if (nHeight > consensusParams.BARLEY_HARVEST_HEIGHT)
        {
...
bool BlockManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (!CheckBlockHeader(block, hash, state, chainparams.GetConsensus(), -1)) {
        ...
        // BIBLEPAY
        if (!ContextualCheckBlockMinedBySanc(block)) {
            return state.Invalid(BlockValidationResult::BLOCK_CACHED_INVALID, "bad-solution");
        }
        // END OF BIBLEPAY
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, m_mnhfManager, *m_quorum_block_processor, *m_clhandler, m_params.GetConsensus(), inputs, false /*fJustCheck*/, false /*fScriptChecks*/, state, mnlist_updates_opt)) {
        return error("RollforwardBlock(BIBLEPAY): ProcessSpecialTxsInBlock for block %s failed with %s",
            pindex->GetBlockHash().ToString(), state.ToString());
    }
    ...
    if (fAddressIndex) {
        if (!pblocktree->WriteAddressIndex(addressIndex)) {
            return error("RollforwardBlock(BIBLEPAY): Failed to write address index");
        }

        if (!pblocktree->UpdateAddressUnspentIndex(addressUnspentIndex)) {
            return error("RollforwardBlock(BIBLEPAY): Failed to write address unspent index");
        }
    }

    if (fSpentIndex) {
        if (!pblocktree->UpdateSpentIndex(spentIndex))
            return error("RollforwardBlock(BIBLEPAY): Failed to write transaction index");
    }

    if (fTimestampIndex) {
        if (!pblocktree->WriteTimestampIndex(CTimestampIndexKey(pindex->nTime, pindex->GetBlockHash())))
            return error("RollforwardBlock(BIBLEPAY): Failed to write timestamp index");
    }
...
bool CChainState::ReplayBlocks()
{
    ...
    if (!hashHeads[1].IsNull()) { // The old tip is allowed to be 0, indicating it's the first flush.
        ...
        if (fDIP0003Active && !m_evoDb.VerifyBestBlock(pindexOld->GetBlockHash())) {
            return error("ReplayBlocks(BIBLEPAY): Found EvoDB inconsistency");
        }
    }
