...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Sendmany with explicit fee (BIBLEPAY/kB)
        ...
        # This will raise an exception for attempting to get the private key of an invalid BiblePay address
        assert_raises_rpc_error(-5, "Invalid BiblePay address", self.nodes[0].dumpprivkey, "invalid")

        # This will raise an exception for attempting to set a label for an invalid BiblePay address
        assert_raises_rpc_error(-5, "Invalid BiblePay address", self.nodes[0].setlabel, "invalid address", "label")

        # This will raise an exception for importing an invalid address
        assert_raises_rpc_error(-5, "Invalid BiblePay address or script", self.nodes[0].importaddress, "invalid")
        ...
        # send with explicit biblepay/kb fee
        self.log.info("test explicit fee (sendtoaddress as biblepay/kb)")
        self.nodes[0].generate(1)
        self.sync_all(self.nodes[0:3])
