...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf("%s biblepay-tx utility version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
            "Usage:\n"
              "  biblepay-tx [options] <hex-tx> [commands]  Update hex-encoded biblepay transaction\n" +
              "  biblepay-tx [options] -create [commands]   Create hex-encoded biblepay transaction\n" +
              "\n";
