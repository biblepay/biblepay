#!/usr/bin/env python3
# Copyright (c) 2022-2023 The BiblePay Core developers
...
def get_label(pr_num):
    return requests.get(f'https://api.github.com/repos/biblepay/biblepay/pulls/{pr_num}').json()['head']['label']
...
def main():
    ...
    for conflict in conflictPrs:
        ...
        r = requests.get(f'https://api.github.com/repos/biblepay/biblepay/pulls/{this_pr_num}')
        ...
        r = requests.get(f'https://github.com/biblepay/biblepay/branches/pre_mergeable/{our_pr_label}...{get_label(this_pr_num)}')
