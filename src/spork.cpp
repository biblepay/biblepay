...
void CSporkManager::ProcessSpork(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (strCommand == NetMsgType::SPORK) {
		...
		{
		    LOCK(cs_main);
            LogPrint(BCLog::SPORK, "CSporkManager::ProcessSpork1 -- ERROR: invalid signature\n");
			//            Misbehaving(pfrom->GetId(), 100);
            return;
       
		}
		else if (!setSporkPubKeyIDs.count(keyIDSigner)) 
		{
            LOCK(cs_main);
			// LogPrint(BCLog::SPORK, "CSporkManager::ProcessSpork2 -- ERROR: invalid signature\n");
			// Misbehaving(pfrom->GetId(), 100);
            return;
        }
