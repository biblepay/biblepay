...
impl PrivateKey {
    ...
    pub fn generate_biblepay(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "biblepay_helpers")]
    pub fn generate_biblepay_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_biblepay(rng))
            .collect()
    }
