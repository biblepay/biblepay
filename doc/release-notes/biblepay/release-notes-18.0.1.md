...
been added to support BiblePay Platform. This update introduces versioning to
...
InstantSend are provided in [DIP-0022](https://github.com/biblepay/dips/blob/master/dip-0022.md).

Deterministic InstantSend will be activated with the DIP0024 hard fork.

Governance
----------
Several improvements have been made to Biblepay’s DAO governance system.
The governance proposal fee has been reduced from 5 BiblePay to 1 BiblePay following
...
system are provided in [DIP-0023](https://github.com/biblepay/dips/blob/master/dip-0023.md).
...
`salvage` command in the `biblepay-wallet` tool.
...
- The `testnet` field in `biblepay-cli -getinfo` has been renamed to `chain` and
...
deprecated. `size` will only be returned if `biblepayd` is started with
`-deprecatedrpc=size`.
...
---------------------------
This release introduces over 1000 updates from Bitcoin v0.18/v0.19/v0.20 as well as numerous updates from Bitcoin v0.21 and more recent versions. This includes multi-wallet support in the GUI, support for partially signed transactions (PSBT), Tor version 3 support, and a number of other updates that will benefit BiblePay users. Bitcoin changes that do not align with Biblepay’s product needs, such as SegWit and RBF, are excluded from our backporting. For additional detail on what’s included in Bitcoin, please refer to their release notes – v0.18, v0.19, v0.20.
...
See detailed [set of changes](https://github.com/biblepay/biblepay/compare/v0.17.0.3...biblepay:v18.0.1).
