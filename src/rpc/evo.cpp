    ...
    static const std::map<std::string, RPCArg> mapParamHelp = {
        {"collateralAddress",
            {"collateralAddress", RPCArg::Type::STR, RPCArg::Optional::NO,
                "The BiblePay address to send the collateral to."}
        },
        {"collateralHash",
        ...
        {"ownerAddress",
            {"ownerAddress", RPCArg::Type::STR, RPCArg::Optional::NO,
                "The BiblePay address to use for payee updates and proposal voting.\n"
        ...
        {"payoutAddress_register",
            {"payoutAddress", RPCArg::Type::STR, RPCArg::Optional::NO,
                "The BiblePay address to use for masternode reward payments."}
        },
        ...
        {"platformP2PPort",
            {"platformP2PPort", RPCArg::Type::NUM, RPCArg::Optional::NO,
                "TCP port of BiblePay Platform peer-to-peer communication between nodes (network byte order)."}
        },
        {"platformHTTPPort",
...
static void protx_register_fund_evo_help(const JSONRPCRequest& request)
{
    RPCHelpMan{
        ...
        "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 4000 Biblepay\n"
        "to the address specified by collateralAddress and will then function as the collateral of your\n"
        "EvoNode.\n"
        "A few of the limitations you see in the arguments are temporary and might be lifted after DIP3\n"
        "is fully deployed.\n" +
...
UniValue protx_register_common_wrapper(const JSONRPCRequest& request,
                                              ...
                                              const MnType mnType)
{
    const bool isEvoRequested = mnType == MnType::Temple;
    if (isEvoRequested) {
...
UniValue protx_register_evo(const JSONRPCRequest& request, const ChainstateManager& chainman)
{
    ...
    return protx_register_common_wrapper(request, chainman, false, isExternalRegister, isFundRegister, isPrepareRegister, MnType::Temple);
}
...
static UniValue protx_update_service_common_wrapper(const JSONRPCRequest& request, const ChainstateManager& chainman, const MnType mnType)
{
    ...
    if (!request.params[paramIdx + 1].isNull()) {
        ...
        if (!IsValidDestination(feeSource))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid BiblePay address: ") + request.params[paramIdx + 1].get_str());
    } else {
...
static UniValue protx_revoke(const JSONRPCRequest& request, const ChainstateManager& chainman)
{
    ...
    if (!request.params[3].isNull()) {
        ...
        if (!IsValidDestination(feeSourceDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid BiblePay address: ") + request.params[3].get_str());
        FundSpecialTx(wallet.get(), tx, ptx, feeSourceDest);
...
static UniValue protx_list(const JSONRPCRequest& request, const ChainstateManager& chainman)
{
    ...
    if (type == "wallet") {
        ...
        mnList.ForEachMN(onlyValid, [&](const auto& dmn) {
            if (onlyEvoNodes && dmn.nType != MnType::Temple) return;
            ret.push_back(BuildDMNListEntry(wallet.get(), dmn, *node.mn_metaman, detailed, chainman));
        });
    } else {
...
UniValue protx(const JSONRPCRequest& request)
{
    ...
    if (command == "protxregister" || command == "protxregister_fund" || command == "protxregister_prepare") {
        ...
        return protx_update_service_common_wrapper(new_request, chainman, MnType::Temple);
