...
Currently, 10% of block rewards are set aside for the BiblePay DAO treasury which
...
The asset lock process provides a mechanism for locking and unlocking BiblePay in a
credit pool. This credit pool mirrors the available credit supply in Biblepay
Platform. When a certain amount of BiblePay is locked, a proportional number of
credits are issued to the identity specified by the transaction issuer. The
identity then uses the credits to pay fees for operations done on Biblepay
Platform. Masternode identities receive the fees and can convert them back to
BiblePay by withdrawing them from BiblePay Platform.
...
### Removal of reject network messages from BiblePay Core (BIP61)

This feature has been disabled by default since BiblePay Core version 19.0.0.
Nodes on the network can not generally be trusted to send valid ("reject")
messages, so this should only ever be used when connected to a trusted node.

Since BiblePay Core version 20.0.0 there are extra changes:
...
- It is possible to run BiblePay Core as an I2P service and connect to such
...
- The `fundrawtransaction`, `sendmany`, `sendtoaddress`, and
  ...
  fee estimation methods, `BIBLEPAY/kB` and `duff/B`. The target is the fee
...
- `testmempoolaccept` and `sendrawtransaction` no longer return the P2P
  ...
  Section [_Removal of reject network messages from BiblePay Core (BIP61)_](#removal-of-reject-network-messages-from-biblepay-core-bip61) above for
...
### Biblepay-specific changes in existing RPCs:
...
- The `biblepay-cli -getinfo` command now displays the wallet name and balance for
...
Please check `Help -> Command-line options` in Qt wallet or `biblepayd --help` for
more information.
...
python test framework and `biblepay-cli` tool both take advantage of this, so
for example:

```sh
biblepay-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
biblepay-cli -named createwallet mywallet load_on_startup=1
```
...
with Biblepay’s product needs, such as SegWit and RBF, are excluded from our
...
[DIP-0023]: https://github.com/biblepay/dips/blob/master/dip-0023.md
[DIP-0029]: https://github.com/biblepay/dips/blob/master/dip-0029.md

[i2p]: https://github.com/biblepay/biblepay/blob/v20.x/doc/i2p.md
[guix-readme]: https://github.com/biblepay/biblepay/blob/v20.x/contrib/guix/README.md
[json-rpc]: https://github.com/biblepay/biblepay/blob/v20.x/doc/JSON-RPC-interface.md#parameter-passing
[set-of-changes]: https://github.com/biblepay/biblepay/compare/v19.3.0...biblepay:v20.0.0
