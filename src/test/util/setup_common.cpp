...
void BiblepayTestSetup(NodeContext& node)
{
...
void BiblepayTestSetupClose(NodeContext& node)
{
...
CBlock TestChainSetup::CreateBlock(const std::vector<CMutableTransaction>& txns, const CScript& scriptPubKey)
{
    ...
    while (!CheckProofOfWork(block.GetHash(), block.nBits, chainparams.GetConsensus(), block.nVersion, block.nTime)) ++block.nNonce;
