# Bootstrappable BiblePay Core Builds

This directory contains the files necessary to perform bootstrappable Biblepay
Core builds.
...
To build BiblePay Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      BiblePay Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/biblepay-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/biblepay/guix.sigs).
