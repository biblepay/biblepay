...
/**
 * Fees smaller than this (in pence) are considered zero fee (for transaction creation)
...
bool CWallet::AccountMove2(std::string strFrom, std::string strTo, CAmount nAmount, std::string strComment)
{
    WalletBatch batch(*database);
    if (!batch.TxnBegin())
        return false;
            ...
            while (--nMaxTries > 0)
            {
                ...
                if (getChange() < 0) {
                    if (nSubtractFeeFromAmount == 0) {
                        ...
                        // so just add 1 pence to fail in the next loop step with a correct reason
...
std::string CWallet::GenerateNewAddress(std::string& sError, std::string sName)
{
	if (!IsLocked(true))
		TopUpKeyPool();
	...
	SetAddressBook(keyID, sName, "receive"); //receive == visible in address book, hidden = non-visible
	//LogPrintf(" created new address %s ", CBitcoinAddress(keyID).ToString().c_str());
    return EncodeDestination(keyID);
}

std::string CWallet::SignBBPUTXO(std::string sUTXO, std::string& sError)
{
	CAmount nValue = 0;
	std::string sErr;
	std::string sAddress = GetUTXO(sUTXO, -1, nValue, sErr);
	...
	CTxDestination destAddr = DecodeDestination(sAddress);
	bool isValid = IsValidDestination(destAddr);

	if (!isValid) 
	{
		sError = "Invalid UTXO address";
		return "";
	}
...
/*
bool CWallet::VerifyNonstandardSignature(std::string sAddress, std::string sTargMsg, std::string sSig, int nKeyType)
{
...
bool CWallet::CheckStakeSignature(std::string sBitcoinAddress, std::string sSignature, std::string strMessage, std::string& strError)
{
