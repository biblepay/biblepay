...
static CMutableTransaction CreateProRegTx(SimpleUTXOMap& utxos, int port, const CScript& scriptPayout, const CKey& coinbaseKey, CKey& ownerKeyRet, CBLSSecretKey& operatorKeyRet)
{
    ...
    FundTransaction(tx, utxos, scriptPayout, SANCTUARY_COLLATERAL * COIN, coinbaseKey);
...
BOOST_FIXTURE_TEST_CASE(dip3_test_mempool_reorg, TestChainDIP3Setup)
{
    ...
    FundTransaction(tx_collateral, utxos, scriptCollateral, SANCTUARY_COLLATERAL * COIN, coinbaseKey);
    ...
    for (size_t i = 0; i < tx_collateral.vout.size(); ++i) {
        if (tx_collateral.vout[i].nValue == SANCTUARY_COLLATERAL * COIN) {
    ...
    FundTransaction(tx_reg, utxos, scriptPayout, SANCTUARY_COLLATERAL * COIN, coinbaseKey);
...
BOOST_FIXTURE_TEST_CASE(dip3_test_mempool_dual_proregtx, TestChainDIP3Setup)
{
    ...
    for (size_t i = 0; i < tx_reg1.vout.size(); ++i) {
        if (tx_reg1.vout[i].nValue == SANCTUARY_COLLATERAL * COIN) {
    ...
    FundTransaction(tx_reg2, utxos, scriptPayout, SANCTUARY_COLLATERAL * COIN, coinbaseKey);
