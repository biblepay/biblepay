...
[[nodiscard]] static bool IsTransactionValid(const CTransaction& txNew, const CBlockIndex* const pindexPrev, const CAmount blockSubsidy, const CAmount feeReward)
{
    ...
    if (nBlockHeight < Params().GetConsensus().BABYLON_FALLING_HEIGHT) {
        // BBP blocks before this height have nonstandard and non deterministic payment lists
        return true;
    }
...
bool IsBlockValueValid(const CSporkManager& sporkManager, CGovernanceManager& governanceManager,
                       const CMasternodeSync& mn_sync, const CBlock& block, const int nBlockHeight, CAmount blockReward, std::string& strErrorRet)
{
