...
class TestNode():
    ...
    def wait_for_rpc_connection(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcassword. biblepayd still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to biblepayd")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, partial_match=False, *args, **kwargs):
        ...
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                self.log.debug('biblepayd failed to start: %s', e)
                self.running = False
                self.process = None
...
class TestNodeCLI():
    ...
    def send_cli(self, command=None, *args, **kwargs):
        ...
        assert not (pos_args and named_args), "Cannot use positional arguments and named arguments in the same biblepay-cli call"
        ...
        self.log.debug("Running biblepay-cli command: %s" % command)
