...
void TestPackageSelection(const CChainParams& chainparams, const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst) EXCLUSIVE_LOCKS_REQUIRED(::mempool.cs)
{
    ...
    pblocktemplate = AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey, "", uRXKey, vchRXHeader);
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    BOOST_CHECK(pemptyblocktemplate = AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey, "", uRXKey, vchRXHeader));
    ...
    auto createAndProcessEmptyBlock = [&]() {
        ...
        CBlock *pblock = &pemptyblocktemplate->block; // pointer for convenience
        {
            ...
            while (!CheckProofOfWork(pblock->GetHash(), pblock->nBits, chainparams.GetConsensus(), chainActive.Tip()->nHeight, pblock->GetHash().GetHex(), uint256S("0x0"), 2, pblock->nTime)) {
                pblock->nNonce++;
            }
        }
    ...
    BOOST_CHECK(pblocktemplate = AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey, "", uRXKey, vchRXHeader));
    ...
    BOOST_CHECK_EXCEPTION(AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey, "", uRXKey, vchRXHeader), std::runtime_error, HasReason("bad-blk-sigops"));
    mempool.clear();
    ...
    BOOST_CHECK_EXCEPTION(AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey, "", uRXKey, vchRXHeader), std::runtime_error, HasReason("bad-txns-inputs-missingorspent"));
    mempool.clear();
    ...
    BOOST_CHECK_EXCEPTION(AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey, "", uRXKey, vchRXHeader), std::runtime_error, HasReason("bad-cb-multiple"));
    mempool.clear();
    ...
    BOOST_CHECK_EXCEPTION(AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey, "", uRXKey, vchRXHeader), std::runtime_error, HasReason("block-validation-failed"));
    mempool.clear();
